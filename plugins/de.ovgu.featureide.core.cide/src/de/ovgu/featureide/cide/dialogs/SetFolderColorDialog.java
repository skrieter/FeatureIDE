...
public class SetFolderColorDialog {
	...
	public ArrayList<String> open(IFile file) {
		 ...
		 * fileName); listDialog.setContentProvider(ArrayContentProvider.getInstance()); listDialog.setLabelProvider(new LabelProvider());
		 * 
		 * Vector<String> featureList = new Vector<String>(); IFeatureProject featureProject = null;
		 * 
		 * featureProject = CorePlugin.getFeatureProject(file);
		 * 
		 * if (featureProject != null) { for (String feature : featureProject.getFeatureModel().getConcreteFeatureNames()) { featureList.add(feature); } }
		 * 
